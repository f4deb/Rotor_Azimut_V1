...
void i2c_dev_gy511_init(void)
{
    esp_err_t ret = esp_io_expander_new_i2c_gy511(getI2cBus(), ESP_IO_EXPANDER_I2C_PCF8574_ADDRESS_000, &io_expander);
    if (ESP_OK !=  ret){
        ESP_LOGE(TAG, "GY511 create returned error");
    }
    else if (ESP_OK ==  ret){
        ESP_LOGI(TAG, "GY511 create returned succed");
    }
}

void i2c_dev_gy511_deinit(void)
{
    esp_err_t ret = esp_io_expander_del(io_expander);
    if (ESP_OK !=  ret){
        ESP_LOGE(TAG, "GY511 delete returned error");
    }
    else if (ESP_OK ==  ret){
        ESP_LOGI(TAG, "GY511 delete returned succed");
    }
}
